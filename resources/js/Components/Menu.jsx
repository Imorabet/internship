...
export default function Menu({ className ="", ...props }) {
    return (
        <div
            {...props}
            className={`flex justify-center justify-items-center `+className}
        >
            ...
            <Link
                href={route("login")}
                className=" right-0 font-semibold uppercase text-gray-600 bg-white p-3 rounded-md focus:outline focus:outline-2 focus:rounded-sm focus:outline-slate-400"
            >
                Se connecter
            </Link>
        </div>
